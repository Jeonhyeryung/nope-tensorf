...
from tensorf.tensoRF import TensorCP, TensorVM, TensorVMSplit
...
def train(cfg):
    ...
    if network_type=='official':
        ...
        # bounding box
        # scene_bbox = torch.from_numpy(np.loadtxt(f'{data_dir}/bbox.txt')).float()[:6].view(2,3)*1.2
        scene_bbox = torch.tensor(cfg['dataloading']['bbox']).float()[:6].view(2,3)*1.2 # 이거 형식 list로 잘 읽히려나..? error?
        ...
        reso_cur = N_to_reso(cfg['tensorf']['N_voxel_init'], aabb)

        # tensorf rendering sampling 
        model = eval( cfg['tensorf']['model_name'])(aabb, reso_cur, device,
                    density_n_comp=cfg['tensorf']['n_lamb_sigma'], appearance_n_comp=cfg['tensorf']['n_lamb_sh'], 
                    app_dim= cfg['tensorf']['data_dim_color'], near_far=cfg['tensorf']['near_far'],
                    shadingMode= cfg['tensorf']['shading_mode'], alphaMask_thres = cfg['tensorf']['alpha_mask_thre'],
                    density_shift= cfg['tensorf']['density_shift'], distance_scale=cfg['tensorf']['distance_scale'],
                    pos_pe= cfg['tensorf']['pos_pe'], view_pe=cfg['tensorf']['view_pe'], fea_pe= cfg['tensorf']['fea_pe'],
                    featureC= cfg['tensorf']['featureC'], step_ratio= cfg['tensorf']['step_ratio'], fea2denseAct= cfg['tensorf']['fea2denseAct'])
