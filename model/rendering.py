...
class Renderer(nn.Module):
    ...
    def forward(self, pixels, depth, camera_mat, world_mat, scale_mat, 
                      rendering_technique = 'tensorf', add_noise=True, eval_=False,
                     it=1000000):
        if rendering_technique == 'nope_nerf':
            ...
            out_dict = self.tensorf(pixels, depth, camera_mat, world_mat, 
                scale_mat, it=it, add_noise=add_noise, eval_=eval_
            )
        elif rendering_technique == 'tensorf_renderer':
            out_dict = self.tensorf_renderer(pixels, depth, camera_mat, world_mat, 
                scale_mat, it=it, add_noise=add_noise, eval_=eval_
            )    
        return out_dict
    ...
    def tensorf_renderer(self, pixels, depth, camera_mat, world_mat, 
            ...
            # Get mask for predicted depth -> deleting illegal values(e.g. inf, False)
            ...
            # Run TensoRF
            ###########################################################
            # Q. ray_vect normalize? -> cfg['rendering'][normalise_ray] = true
            ...
            # ##### tensorf output depth or distance?? 아래 참조 애매...########
    ...
    def tensorf(self, pixels, depth, camera_mat, world_mat, 
        ...
        out_dict = {
            'rgb': rgb.reshape(batch_size, -1, 3),
            'z_vals': z_val.squeeze(-1), # Not necessary
            'normal': None, # not necessary -> default config  == false
    ...
    def nope_nerf(self, pixels, depth, camera_mat, world_mat, 
        ...
        out_dict = {
            ...
            'z_vals': z_val.squeeze(-1), # Not necessary
            'normal': diff_norm, # not necessary -> default config  == false
            'depth_pred': dist_rendered_masked, # for loss
            'depth_gt': dist_dpt_masked,  # for loss
            'alpha': alpha
        }
    ...
    def ray_marching(self, ray0, ray_direction, model, c=None,
        ...
        with torch.no_grad():
            # sigma_feature = self.model.compute_densityfeature(xyz_sampled)
            # validsigma = self.model.feature2density(sigma_feature)
            ...
            for i in a:
                ...
                j+=1
            val = torch.cat([(
                self.model.feature2density(self.model.compute_densityfeature(p_split)) - tau)
